...
def processar_parte(linhas):
    ...
    parte_altura = linhas[1] - linhas[0]
    ...
    parte_imagem.write_to_file(f'parte_{linhas[0]}_{linhas[1]}.tiff')
...
if __name__ == "__main__":
    ...
    # Gerando os intervalos de linhas para cada parte
    linhas = [(i*parte_altura, (i+1)*parte_altura) for i in range(num_partes)]

    # Usando Pool para paralelizar o processamento das partes da imagem
    with multiprocessing.Pool(processes=16) as pool:
        results = pool.map(processar_parte, linhas)
